<?php
function handle_tuyen_dung_filter() {
    ...
    if ($query->have_posts()) {
        $first_post = $query->posts[0]; // Đây là bài viết đầu tiên
            ...
            <?php
                foreach ($pagination as $page_link):
                    ...
                    if (strpos($page_link, '<a') !== false) {
                        // Bỏ toàn bộ query string: cắt từ dấu "?" trở đi trong href
...
// Đăng ký action AJAX
add_action('wp_ajax_filter_tuyendung', 'handle_tuyen_dung_filter');
add_action('wp_ajax_nopriv_filter_tuyendung', 'handle_tuyen_dung_filter');