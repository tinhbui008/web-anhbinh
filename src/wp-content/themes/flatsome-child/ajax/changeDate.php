<?php
add_action( 'wp_ajax_changeDate', 'changeDate_init' );
add_action( 'wp_ajax_nopriv_changeDate', 'changeDate_init' );
function changeDate_init() {
    ob_start();

    //do bên js để dạng json nên giá trị trả về dùng phải encode
    $today = (isset($_POST['dateCurrent']))?esc_attr($_POST['dateCurrent']) : '';
    $type = (isset($_POST['type']))?esc_attr($_POST['type']) : '';
    //$today = $dateCurrent;//current_time('Y-m-d');
    $today_start = $today . ' 00:00:00';
    $today_end = date('Y-m-d', strtotime($today . ' +6 days'));
    ...
    $year_tomorrow = date('Y', strtotime($today . '+6 days'));
    ...
    $Date = getDatesFromRange($today, date('Y-m-d', strtotime($today . '+6 days')));
...
?>
    ...
    <div class="s-sukien-tab">
        ...
        <div class="s-sukien-timeNow"><?=date('d/m', $now)?><?=($year_tomorrow>$year_current) ? '/'.$year_current : ''?> - <?=date('d/m/Y', strtotime($today . '+6 days'))?></div>
        <!-- NEXT DATE -->
    ...
    <div id="s-sukien-containBox">
        ...
        <div class="<?=(!empty($events_by_date)) ? 's-sukien-dates-main' : 's-sukien-dates-main'?>" id="gridContainer">
                                    ...
                                    <div class="s-sukien-itemsub-info" style="display:none">
                                        <div class="s-sukien-itemsub-infoCopy" id="tooltip-<?=$event['id']?>">
                                            <div class="s-sukien-itemsub-top scrollBar-cs2">
                                                ...
                                                <p class="s-sukien-itemsub-date">15:30-17:30, 25/12/2024</p>
            ...
            <?php /*
            <?php if(empty($events_by_date)):?>
                <div class="s-sukien-list-dev">
                    <?php for($i=0;$i<28;$i++): ?>
                        <div class="s-sukien-item s-sukien-item-dev"></div>
                    <?php endfor;?>
                </div>                
                <div class="s-no-events">Không có sự kiện nào được tìm thấy</div>
            <?php endif;?>*/ ?>
        </div>
    </div>
...
<?php
    $html = ob_get_clean();
    echo $html;
    //wp_send_json_success('Chào mừng bạn đến với '.$dateCurrent);
